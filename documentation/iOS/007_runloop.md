...
event loop, message dispatcher, message loop, message pump, run loop 是一个程序结构, 用于等待、分发事件或消息.
...
// 没有对外暴露，只是通过 CFRunLoopRef 的接口进行了封装。
CFRunLoopModeRef
...
线程和 runloop 是一一对应的, 其关系是保存在一个全局的 Dictionary 里。线程刚创建时并没有 RunLoop，如果你不主动获取，那它一直都不会有。RunLoop 的创建是发生在第一次获取时，RunLoop 的销毁是发生在线程结束时。
...
[Mac OS X 背后的故事（三）Mach之父Avie Tevanian](http://history.programmer.com.cn/8121/)

Accent -> Mach -> BSD、IOKit、Mach osfmk -> FreeBSD -> PPC64,x86,x86_64,ARM 

### ref

[CFRunLoopRef 源码](https://opensource.apple.com/source/CF/CF-635.19/CFRunLoop.c.auto.html)
